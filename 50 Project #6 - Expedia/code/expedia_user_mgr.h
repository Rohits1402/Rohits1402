...
public:
	...
	void LoadDatabase()
	{
		...
		admin_user->SetUserName("rohit");
		admin_user->SetPassword("111");
		admin_user->SetEmail("rohit@gmail.com");
		// admin_user->SetIsLibraryAdmin(false);
		admin_user->SetName("Rohit Chauhan");
		userame_userobject_map[admin_user->GetUserName()] = admin_user;

		Customer *customrUser = new Customer();
		customrUser->SetUserName("rohit");
		customrUser->SetPassword("1402");
		customrUser->SetEmail("rschauhan1402@gmail.com");
		// user1->SetIsLibraryAdmin(true);
		customrUser->SetName("Rohit Singh Chauhan");
